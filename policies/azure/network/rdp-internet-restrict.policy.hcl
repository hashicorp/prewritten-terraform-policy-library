...
resource_policy "azurerm_network_security_group" "restrict_rdp_from_internet" {
  locals {
    ...
    internet_source_rules = [for rule in local.inbound_allow_rdp_protocol_rules : rule if core::contains(["0.0.0.0/0", "Internet", "*", "Any"], core::try(rule.source_address_prefix, "")) || core::length([for source in (core::try(rule.source_address_prefixes, null) != null ? core::try(rule.source_address_prefixes, []) : []) : source if core::contains(["0.0.0.0/0", "Internet", "*", "Any"], source)]) > 0]
