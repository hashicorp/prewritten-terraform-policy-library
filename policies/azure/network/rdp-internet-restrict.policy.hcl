...
resource_policy "azurerm_network_security_group" "restrict_rdp_from_internet" {
  locals {
    ...
    inbound_allow_rdp_protocol_rules = [for rule in local.security_rules : rule if core::lower(core::try(rule.direction, "")) == "inbound" && core::lower(core::try(rule.access, "")) == "allow" && core::contains(["tcp", "*"], core::lower(core::try(rule.protocol, "")))]

    internet_source_rules = [for rule in local.inbound_allow_rdp_protocol_rules : rule if core::contains(["0.0.0.0/0", "internet", "*", "any"], core::lower(core::try(rule.source_address_prefix, ""))) || core::length([for source in (core::try(rule.source_address_prefixes, null) != null ? core::try(rule.source_address_prefixes, []) : []) : source if core::contains(["0.0.0.0/0", "internet", "*", "any"], core::lower(source))]) > 0]
