...
resource_policy "azurerm_network_security_group" "restrict_internet_udp_ports" {
  locals {
    ...
    internet_exposed_rules = [for rule in local.inbound_udp_allow_rules : rule
      if core::contains(["0.0.0.0/0", "::/0", "Internet", "*"], core::try(rule.source_address_prefix, null) != null ? rule.source_address_prefix : "")
      || core::length([for prefix in (core::try(rule.source_address_prefixes, null) != null ? rule.source_address_prefixes : []) : prefix if core::contains(["0.0.0.0/0", "::/0", "Internet", "*"], prefix)]) > 0
    ]
...
resource_policy "azurerm_network_security_rule" "restrict_internet_udp_ports_standalone" {
  locals {
    ...
    is_internet_source      = core::contains(["0.0.0.0/0", "::/0", "Internet", "*"], local.source_prefix) || core::try(core::length([for prefix in local.source_prefixes : prefix if core::contains(["0.0.0.0/0", "::/0", "Internet", "*"], prefix)]), 0) > 0
