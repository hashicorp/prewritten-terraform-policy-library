...
resource_policy "azurerm_network_security_group" "restrict_internet_udp_ports" {
  locals {
    ...
    internet_exposed_rules  = [for rule in local.inbound_udp_allow_rules : rule if core::contains(["0.0.0.0/0", "0.0.0.0", "Internet", "*", "Any"], core::try(rule.source_address_prefix, null) != null ? core::try(rule.source_address_prefix, "") : "") || core::endswith(core::try(rule.source_address_prefix, null) != null ? core::try(rule.source_address_prefix, "") : "", "/0") || core::length([for prefix in (core::try(rule.source_address_prefixes, null) != null ? core::try(rule.source_address_prefixes, []) : []) : prefix if core::contains(["0.0.0.0/0", "0.0.0.0", "Internet", "*", "Any"], prefix) || core::endswith(prefix, "/0")]) > 0]
...
resource_policy "azurerm_network_security_rule" "restrict_internet_udp_ports_standalone" {
  locals {
    ...
    is_internet_source      = core::contains(["0.0.0.0/0", "0.0.0.0", "Internet", "*", "Any"], local.source_prefix) || core::endswith(local.source_prefix, "/0") || core::length([for prefix in local.source_prefixes : prefix if core::contains(["0.0.0.0/0", "0.0.0.0", "Internet", "*", "Any"], prefix) || core::endswith(prefix, "/0")]) > 0
