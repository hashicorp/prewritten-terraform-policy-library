...
resource_policy "aws_redshift_cluster" "kms-enabled" {
    ...
    locals {
        ...
        kms_key_arns_provided = input.redshift_kms_key_arns != ""
        valid_kms_key_arn = local.kms_key_arns_provided ? (local.kms_key_arn != "" ? core::contains(core::split(",", input.redshift_kms_key_arns), local.kms_key_arn) : true) : true
    }
