...
resource_policy "aws_sfn_state_machine" "logging_enabled" {
    ...
    locals {
        ...
        logging_config_list_raw = core::try(attrs.logging_configuration, null)
        logging_config_list = local.logging_config_list_raw != null ? local.logging_config_list_raw : []
        has_logging_config = core::length(local.logging_config_list) > 0
        ...
        has_valid_level = local.has_logging_config && core::contains(local.valid_levels, local.log_level)
