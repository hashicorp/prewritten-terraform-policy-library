...
policytest {
    targets = [
        "waf-global-webacl-not-empty.policy.hcl"
    ]
}

# Test 1: PASS - Web ACL with one rule configured
resource "aws_waf_web_acl" "pass_with_single_rule" {
  attrs = {
...
resource "aws_waf_web_acl" "fail_with_empty_rules" {
  expect_failure = true
  attrs = {
    name        = "empty-web-acl"
...
resource "aws_waf_web_acl" "pass_with_multiple_rules" {
  attrs = {
...
resource "aws_waf_web_acl" "fail_with_no_rules" {
  expect_failure = true
  attrs = {
    name        = "empty-web-acl"
    metric_name = "emptyWebAcl"
