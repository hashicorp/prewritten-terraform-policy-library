...
policytest {
  targets = ["cloudwatch-network-gateway-changes-alarm.policy.hcl"]
}
...
resource "aws_sns_topic" "pass_gateway_changes_topic" {
  attrs = {
...
resource "aws_cloudwatch_log_metric_filter" "pass_gateway_changes_filter" {
  attrs = {
...
resource "aws_cloudwatch_metric_alarm" "pass_gateway_changes_alarm" {
  attrs = {
...
resource "aws_sns_topic_subscription" "pass_gateway_changes_subscription" {
  attrs = {
...
# Test 5: FAIL - filter pattern missing CreateInternetGateway
resource "aws_cloudwatch_log_metric_filter" "fail_missing_create_igw" {
  expect_failure = true
  attrs = {
    name           = "missing-create-igw"
    pattern        = "{($.eventName=AttachInternetGateway) || ($.eventName=DeleteInternetGateway)}"
    log_group_name = "cloudtrail-events"
    metric_transformation = [{
      name          = "MissingCreateIGW"
      namespace     = "SecurityMetrics"
      value         = "1"
      default_value = "0"
    }]
...
# Test 6: FAIL - filter pattern missing AttachInternetGateway
resource "aws_cloudwatch_log_metric_filter" "fail_missing_attach_igw" {
  expect_failure = true
  attrs = {
    name           = "missing-attach-igw"
    pattern        = "{($.eventName=CreateInternetGateway) || ($.eventName=DeleteInternetGateway)}"
    log_group_name = "cloudtrail-events"
    metric_transformation = [{
      name          = "MissingAttachIGW"
      namespace     = "SecurityMetrics"
      value         = "1"
      default_value = "0"
    }]
...
resource "aws_cloudwatch_log_metric_filter" "fail_missing_default_value" {
  ...
  attrs = {
    ...
    log_group_name = "cloudtrail-events"
    metric_transformation = [{
      name      = "NetworkGatewayChangesNoDefault"
      namespace = "SecurityMetrics"
      value     = "1"
    }]
  }
...
resource "aws_cloudtrail" "pass_network_gateway_changes_trail" {
  attrs = {
    name                          = "network-gateway-changes-trail"
