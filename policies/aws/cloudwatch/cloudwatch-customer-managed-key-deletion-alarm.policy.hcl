...
input "cloudwatch-customer-managed-key-deletion-alarm-enforcement-level" {
  type    = string
  default = "advisory"
}

locals {
  kms_all_metric_filters = core::getresources("aws_cloudwatch_log_metric_filter", {})
  ...
  kms_deletion_filters = [
    for f in local.kms_all_metric_filters : f
    if core::length([
      for event in local.kms_required_events : event
      if core::try(core::contains_substring(f.pattern, event), false)
    ]) == core::length(local.kms_required_events) &&
  ...
  kms_deletion_metric_names = [
    for f in local.kms_deletion_filters :
    core::try(f.metric_transformation[0].name, "")
    if core::try(f.metric_transformation[0].name, "") != ""
  ]

  kms_deletion_metric_namespaces = {
    for f in local.kms_deletion_filters :
    core::try(f.metric_transformation[0].name, "") => core::try(f.metric_transformation[0].namespace, "")
    if core::try(f.metric_transformation[0].name, "") != ""
  }

  kms_all_alarms = core::getresources("aws_cloudwatch_metric_alarm", {})

  kms_compliant_alarms = [
    for alarm in local.kms_all_alarms : alarm
    if core::contains(local.kms_deletion_metric_names, core::try(alarm.metric_name, "")) &&
       core::try(local.kms_deletion_metric_namespaces[alarm.metric_name], "") == core::try(alarm.namespace, "") &&
  ...
  kms_all_sns_topics = core::getresources("aws_sns_topic", {})

  kms_declared_topic_arns = [
    for t in local.kms_all_sns_topics :
    core::try(t.arn, "")
    if core::try(t.arn, "") != ""
  ]
}

resource_policy "aws_cloudwatch_log_metric_filter" "kms_deletion_filter" {
  enforcement_level = input.cloudwatch-customer-managed-key-deletion-alarm-enforcement-level
  ...
  locals {
    ...
    detects_kms_deletion = core::length([
      for event in local.kms_required_events : event
      if core::try(core::contains_substring(local.pattern, event), false)
    ]) == core::length(local.kms_required_events)
    is_compliant = local.detects_kms_deletion && local.transformation_name != "" && local.transformation_ns != "" && local.transformation_val == "1" && local.transformation_dval == "0"
  }

  enforce {
    condition     = local.is_compliant
    error_message = "CloudWatch log metric filter must detect KMS key disabling or deletion by including 'kms.amazonaws.com', 'DisableKey', and 'ScheduleKeyDeletion' in the pattern, emit a named metric with a non-empty namespace, value = \"1\", and default_value = \"0\"."
  }
}

resource_policy "aws_cloudwatch_metric_alarm" "kms_deletion_alarm" {
  enforcement_level = input.cloudwatch-customer-managed-key-deletion-alarm-enforcement-level
  ...
  locals {
    ...
    references_correct_metric = core::contains(local.kms_deletion_metric_names, local.metric_name)
    expected_namespace        = core::try(local.kms_deletion_metric_namespaces[local.metric_name], "")
  ...
  enforce {
    condition     = local.is_compliant
    error_message = "CloudWatch metric alarm for KMS key deletion must reference a KMS-deletion metric filter by both metric name and namespace, use threshold 1, use GreaterThanOrEqualToThreshold, and configure at least one alarm action."
  }
}

resource_policy "aws_cloudtrail" "kms_deletion_trail" {
  enforcement_level = input.cloudwatch-customer-managed-key-deletion-alarm-enforcement-level
...
resource_policy "aws_sns_topic_subscription" "kms_deletion_subscription" {
  enforcement_level = input.cloudwatch-customer-managed-key-deletion-alarm-enforcement-level

  locals {
    topic_arn = core::try(attrs.topic_arn, "")
    topic_is_declared = core::contains(local.kms_declared_topic_arns, local.topic_arn)
    matching_alarms = [
      for alarm in local.kms_compliant_alarms : alarm
  ...
  enforce {
    condition     = local.is_compliant
    error_message = "SNS topic subscription must reference a declared aws_sns_topic resource (not a dangling ARN) that is also referenced by a compliant KMS key deletion alarm."
  }
}
