...
input "cloudwatch-iam-policy-changes-alarm-enforcement-level" {
  type    = string
  default = "advisory"
}

locals {
  iam_all_metric_filters = core::getresources("aws_cloudwatch_log_metric_filter", {})
  ...
  iam_policy_changes_filters = [
    for f in local.iam_all_metric_filters : f
    if core::length([
      for event in local.iam_required_events : event
      if core::try(core::contains_substring(f.pattern, event), false)
    ]) == core::length(local.iam_required_events) &&
  ...
  iam_policy_changes_metric_names = [
    for f in local.iam_policy_changes_filters :
    core::try(f.metric_transformation[0].name, "")
    if core::try(f.metric_transformation[0].name, "") != ""
  ]

  iam_policy_changes_metric_namespaces = {
    for f in local.iam_policy_changes_filters :
    core::try(f.metric_transformation[0].name, "") => core::try(f.metric_transformation[0].namespace, "")
    if core::try(f.metric_transformation[0].name, "") != ""
  }

  iam_all_alarms = core::getresources("aws_cloudwatch_metric_alarm", {})

  iam_compliant_alarms = [
    for alarm in local.iam_all_alarms : alarm
    if core::contains(local.iam_policy_changes_metric_names, core::try(alarm.metric_name, "")) &&
       core::try(local.iam_policy_changes_metric_namespaces[alarm.metric_name], "") == core::try(alarm.namespace, "") &&
  ...
  iam_all_sns_topics = core::getresources("aws_sns_topic", {})

  iam_declared_topic_arns = [
    for t in local.iam_all_sns_topics :
    core::try(t.arn, "")
    if core::try(t.arn, "") != ""
  ]
}

resource_policy "aws_cloudwatch_log_metric_filter" "iam_policy_changes_filter" {
  enforcement_level = input.cloudwatch-iam-policy-changes-alarm-enforcement-level
  ...
  locals {
    ...
    detects_iam_changes = core::length([
      for event in local.iam_required_events : event
      if core::try(core::contains_substring(local.pattern, event), false)
    ]) == core::length(local.iam_required_events)
    is_compliant = local.detects_iam_changes && local.transformation_name != "" && local.transformation_ns != "" && local.transformation_val == "1" && local.transformation_dval == "0"
  }

  enforce {
    condition     = local.is_compliant
    error_message = "CloudWatch log metric filter must detect IAM policy changes by including all 14 required IAM policy change events in the pattern, emit a named metric with a non-empty namespace, value = \"1\", and default_value = \"0\"."
  }
}

resource_policy "aws_cloudwatch_metric_alarm" "iam_policy_changes_alarm" {
  enforcement_level = input.cloudwatch-iam-policy-changes-alarm-enforcement-level
  ...
  locals {
    ...
    references_correct_metric = core::contains(local.iam_policy_changes_metric_names, local.metric_name)
    expected_namespace        = core::try(local.iam_policy_changes_metric_namespaces[local.metric_name], "")
  ...
  enforce {
    condition     = local.is_compliant
    error_message = "CloudWatch metric alarm for IAM policy changes must reference an IAM-policy-changes metric filter by both metric name and namespace, use threshold 1, use GreaterThanOrEqualToThreshold, and configure at least one alarm action."
  }
}

resource_policy "aws_cloudtrail" "iam_policy_changes_trail" {
  enforcement_level = input.cloudwatch-iam-policy-changes-alarm-enforcement-level
...
resource_policy "aws_sns_topic_subscription" "iam_policy_changes_subscription" {
  enforcement_level = input.cloudwatch-iam-policy-changes-alarm-enforcement-level

  locals {
    topic_arn         = core::try(attrs.topic_arn, "")
    topic_is_declared = core::contains(local.iam_declared_topic_arns, local.topic_arn)
    matching_alarms   = [
      for alarm in local.iam_compliant_alarms : alarm
  ...
  enforce {
    condition     = local.is_compliant
    error_message = "SNS topic subscription must reference a declared aws_sns_topic resource (not a dangling ARN) that is also referenced by a compliant IAM policy changes alarm."
  }
}
