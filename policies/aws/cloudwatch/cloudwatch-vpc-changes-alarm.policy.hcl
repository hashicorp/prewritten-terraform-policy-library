...
input "cloudwatch-vpc-changes-alarm-enforcement-level" {
  type    = string
  default = "advisory"
}

input "cloudwatch-vpc-changes-alarm-metric-namespace" {
  type    = string
  default = "LogMetrics"
}

locals {
  vpc_changes_metric_alarms       = core::getresources("aws_cloudwatch_metric_alarm", {})
  vpc_changes_sns_topics           = core::getresources("aws_sns_topic", {})
  ...
  vpc_changes_valid_metrics = [
    for filter in local.vpc_changes_valid_metric_filters : {
      name      = core::try(filter.metric_transformation[0].name, "")
      namespace = core::try(filter.metric_transformation[0].namespace, "")
    }
  ]

  vpc_changes_valid_metric_alarms = [
    for alarm in local.vpc_changes_metric_alarms : alarm
    if core::length([
      for metric in local.vpc_changes_valid_metrics : metric
  ...
  vpc_changes_valid_sns_topics = [
    for topic in local.vpc_changes_sns_topics : topic
    if core::length([
      for alarm in local.vpc_changes_valid_metric_alarms : alarm
      if core::contains(
  ...
  vpc_changes_valid_sns_topic_arns = [
    for topic in local.vpc_changes_valid_sns_topics : core::try(topic.arn, "")
  ]
}

resource_policy "aws_cloudtrail" "cloudwatch-vpc-changes-alarm" {
  enforcement_level = input.cloudwatch-vpc-changes-alarm-enforcement-level
  ...
  locals {
    ...
    is_compliant = (
      local.is_multi_region_trail &&
      local.enable_logging &&
      local.cloud_watch_logs_group_arn != "" &&
      local.cloud_watch_logs_role_arn != ""
    )
  }
...
resource_policy "aws_cloudwatch_log_metric_filter" "cloudwatch-vpc-changes-alarm" {
  enforcement_level = input.cloudwatch-vpc-changes-alarm-enforcement-level
  ...
  locals {
    ...
    is_compliant = (
      ...
      local.transformation_default == "0"
    )
  }

  enforce {
    condition = local.is_compliant
    error_message = "An exact VPC-change metric filter using the LogMetrics namespace, with metric value 1 and default value 0, is required."
  }
}

resource_policy "aws_cloudwatch_metric_alarm" "cloudwatch-vpc-changes-alarm" {
  enforcement_level = input.cloudwatch-vpc-changes-alarm-enforcement-level
  ...
  locals {
    ...
    actions_reference_declared_topic = core::length([
      ...
      if core::contains(
        [for t in local.vpc_changes_sns_topics : core::try(t.arn, "")],
        action
      )
    ]) > 0

    is_compliant = (
      local.references_valid_metric &&
      local.comparison_operator == "GreaterThanOrEqualToThreshold" &&
      local.threshold == 1 &&
      local.actions_reference_declared_topic
    )
  }

  enforce {
    condition = local.is_compliant
    error_message = "A CloudWatch alarm referencing the VPC-change metric, using GreaterThanOrEqualToThreshold with a threshold of 1 and at least one alarm action targeting a declared aws_sns_topic resource, is required."
  }
}

resource_policy "aws_sns_topic_subscription" "cloudwatch-vpc-changes-alarm" {
  enforcement_level = input.cloudwatch-vpc-changes-alarm-enforcement-level

  locals {
    topic_arn = core::try(attrs.topic_arn, "")
    is_compliant = local.topic_arn != "" && core::contains(local.vpc_changes_valid_sns_topic_arns, local.topic_arn)
  }

  enforce {
    condition = local.is_compliant
    error_message = "An SNS topic subscription for a topic used by a compliant VPC-change CloudWatch alarm is required."
  }
}
