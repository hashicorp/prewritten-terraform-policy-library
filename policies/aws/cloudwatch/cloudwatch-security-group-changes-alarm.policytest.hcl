...
policytest {
  targets = ["cloudwatch-security-group-changes-alarm.policy.hcl"]
}
...
resource "aws_sns_topic" "pass_sg_changes_topic" {
  attrs = {
...
resource "aws_cloudwatch_log_metric_filter" "pass_sg_changes_filter" {
  attrs = {
...
resource "aws_cloudwatch_metric_alarm" "pass_sg_changes_alarm" {
  attrs = {
...
resource "aws_sns_topic_subscription" "pass_sg_changes_subscription" {
  attrs = {
...
# Test 5: FAIL - filter pattern missing AuthorizeSecurityGroupIngress
resource "aws_cloudwatch_log_metric_filter" "fail_missing_authorize_ingress" {
  expect_failure = true
  attrs = {
    name           = "missing-authorize-ingress"
    pattern        = "{($.eventName=CreateSecurityGroup) || ($.eventName=DeleteSecurityGroup)}"
    log_group_name = "cloudtrail-events"
    metric_transformation = [{
      name          = "MissingAuthorize"
      namespace     = "SecurityMetrics"
      value         = "1"
      default_value = "0"
    }]
...
# Test 6: FAIL - filter pattern missing DeleteSecurityGroup
resource "aws_cloudwatch_log_metric_filter" "fail_missing_delete_sg" {
  expect_failure = true
  attrs = {
    name           = "missing-delete-sg"
    pattern        = "{($.eventName=AuthorizeSecurityGroupIngress) || ($.eventName=CreateSecurityGroup)}"
    log_group_name = "cloudtrail-events"
    metric_transformation = [{
      name          = "MissingDeleteSG"
      namespace     = "SecurityMetrics"
      value         = "1"
      default_value = "0"
    }]
...
resource "aws_cloudwatch_log_metric_filter" "fail_missing_default_value" {
  ...
  attrs = {
    ...
    log_group_name = "cloudtrail-events"
    metric_transformation = [{
      name      = "SecurityGroupChangesNoDefault"
      namespace = "SecurityMetrics"
      value     = "1"
    }]
  }
...
resource "aws_cloudtrail" "pass_sg_changes_trail" {
  attrs = {
    name                          = "sg-changes-trail"
