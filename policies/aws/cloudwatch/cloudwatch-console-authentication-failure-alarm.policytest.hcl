...
resource "aws_sns_topic" "pass_console_auth_failure_topic" {
  attrs = {
    name = "console-auth-failure-alerts"
...
# FAIL - filter pattern missing "Failed authentication"
...
# FAIL - filter pattern missing ConsoleLogin
...
resource "aws_cloudtrail" "pass_console_auth_failure_trail" {
  attrs = {
    name                          = "console-auth-failure-trail"
