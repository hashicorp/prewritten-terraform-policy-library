...
resource_policy "aws_sagemaker_monitoring_schedule" "model_explainability_inter_container_encryption" {
    ...
    locals {
        ...
        is_model_explainability = local.monitoring_type == "ModelExplainability"
    ...
    enforce {
        condition = !local.is_model_explainability || (local.is_model_explainability && local.encryption_enabled == true)
