...
resource_policy "aws_appsync_graphql_api" "field_logging_enabled" {
    ...
    enforce {
        ...
        error_message = "AppSync API does not have log_config defined. Field-level logging must be enabled with field_log_level set to at least the required minimum from input.fieldLoggingLevel (default 'ERROR') and a valid cloudwatch_logs_role_arn to meet AWS Security Hub AppSync.2 requirements. Refer to https://docs.aws.amazon.com/securityhub/latest/userguide/appsync-controls.html#appsync-2 for more details."
    }

    enforce {
        condition = !local.has_log_config || local.has_cloudwatch_logs_role_arn
        error_message = "AppSync API log_config must set a non-empty cloudwatch_logs_role_arn when field-level logging is enabled. Refer to https://docs.aws.amazon.com/securityhub/latest/userguide/appsync-controls.html#appsync-2 for more details."
    }

    # Enforce: Field log level must be ERROR or ALL
    enforce {
        condition = !local.has_log_config || local.has_valid_log_level
        error_message = "AppSync API field_log_level '${local.field_log_level}' does not satisfy the required minimum from input.fieldLoggingLevel='${local.required_log_level}'. Set log_config.field_log_level to '${local.required_log_level}' or a more verbose level. Refer to https://docs.aws.amazon.com/securityhub/latest/userguide/appsync-controls.html#appsync-2 for more details."
    }
