...
The CIS Azure Benchmark requires a minimum retention period of 7 days. Without soft delete, deleted blobs are permanently removed immediately and cannot be recovered. Storage accounts of kind `FileStorage` have no blob service at all and are excluded from enforcement. Accounts of kind `Storage` (GPv1) do support `blob_properties.delete_retention_policy` and are therefore in scope.
