...
This control checks whether a log metric filter and alarm exist for changes to VPCs.

Changes at the VPC level — new VPCs, peering connections, attached or detached gateways — alter the network boundary itself. A new peering connection in particular can link a hardened environment to one that is not.

The policy requires a metric filter matching the VPC and peering lifecycle events, including `CreateVpc`, `DeleteVpc`, `ModifyVpcAttribute`, `AcceptVpcPeeringConnection`, `CreateVpcPeeringConnection`, `DeleteVpcPeeringConnection`, `RejectVpcPeeringConnection`, `AttachClassicLinkVpc`, `DetachClassicLinkVpc`, `DisableVpcClassicLink`, and `EnableVpcClassicLink`. The policy also requires the supporting delivery chain to be in place: a multi-Region CloudTrail trail with logging enabled that delivers events to a CloudWatch Logs group, a metric filter that emits a named metric with `value = "1"` and `default_value = "0"`, an alarm that references that metric by both name and namespace using `GreaterThanOrEqualToThreshold` with a threshold of `1`, and at least one alarm action pointing at a declared SNS topic that has a subscription. A filter without an alarm, or an alarm without a notification target, produces no actionable signal.

This rule is covered by the [cloudwatch-vpc-changes-alarm](https://github.com/hashicorp/prewritten-terraform-policy-library/blob/main/policies/aws/cloudwatch/cloudwatch-vpc-changes-alarm.policy.hcl) policy.

## Policy Results

```bash
trace:
      # cloudwatch-vpc-changes-alarm.policytest.hcl... running
      # resource.aws_cloudtrail.cloudwatch_14_pass... running
      # resource.aws_cloudtrail.cloudwatch_14_pass... pass
      # resource.aws_cloudwatch_log_metric_filter.cloudwatch_14_pass... running
      # resource.aws_cloudwatch_log_metric_filter.cloudwatch_14_pass... pass
      # resource.aws_cloudwatch_metric_alarm.cloudwatch_14_pass... running
      # resource.aws_cloudwatch_metric_alarm.cloudwatch_14_pass... pass
      # resource.aws_sns_topic_subscription.cloudwatch_14_pass... running
      # resource.aws_sns_topic_subscription.cloudwatch_14_pass... pass
      ...
      # cloudwatch-vpc-changes-alarm.policytest.hcl... pass
```
