...
# GAP: Same issue as vpc-endpoint-enabled — see that file for the full explanation.
# The connection subject = "id" / target = "vpc_id" is expressible, but the
# additional service_name regex filter on matched endpoints cannot be expressed
# as a static `where` block (values are not literals — they use regexall). The
# per-match object model (Decision 4) must be confirmed before this can be
# fully converted.
