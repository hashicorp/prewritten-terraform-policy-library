...
resource_policy "aws_rds_cluster" "audit_logging_enabled" {
  ...
  enforce {
    condition     = core::try(attrs.db_cluster_parameter_group_name, "") != ""
  ...
  connected "aws_rds_cluster_parameter_group" {
    ...
    enforce {
      condition     = core::length([for param in core::try(self.parameter, []) : param if param.name == "server_audit_logging" && param.value == "1"]) > 0
      error_message = "Aurora MySQL cluster parameter group must have 'server_audit_logging' set to '1'"
    }

    enforce {
      condition     = core::length([for param in core::try(self.parameter, []) : param if param.name == "server_audit_events" && param.value != ""]) > 0
      error_message = "Aurora MySQL cluster parameter group must have 'server_audit_events' configured with audit events (e.g., 'CONNECT,QUERY,QUERY_DCL,QUERY_DDL,QUERY_DML')"
    }
  }
}
